...
Стек: Python 3.13, OpenCV, numpy, PyQt5, uiautomator2, ADB, MEmu / LDPlayer.
Версия: 1.3.0 (см. [`version.py`](version.py)). Автор: E. Malkov.
...
- Атака (`attacks/attacks.py`): драконы, шары, осада, герои, заклы, герой Duke, Stone Slammer,
  ...
  - Способность героя при низком HP: следит за HP-полоской и жмёт способность, когда просядет.
    ...
    текущий закл/войско.
  - Не тапает вслепую, если прервал бой (End Battle / Pause / Stop).
...
- Многопроцессный мульти-бот (v1.3.0): до N экземпляров в одном GUI-окне, каждый — изолированный
  worker-процесс (`run_from_source.py --worker`, [`worker_run.py`](worker_run.py)) на отдельном
  инстансе эмулятора. Процессная изоляция глобального состояния (`main.host`/`TABS`/`stop_event`/
  `pause_event`) исключает конфликты параллельного управления; коллизии на инстанс предотвращаются
  лок-портом. На каждый бот — своя конфигурация, буфер лога, статистика (`Stats_{N}.json`) и
  управление Start/End/Pause; пауза реализована через файловый IPC-флаг (`profiles/_bot_N.pause`),
  который воркер опрашивает. Раскладка сессии сериализуется в `config/bots_layout.json` с
  опциональным автозапуском при следующем старте.
- Эмуляторы: MEmu (через `memuc`/`.memu`) и LDPlayer 14; эмулятор и инстанс выбираются при старте,
  инстансы исполняются параллельно (по процессу на бота), работа в фоне / свёрнутым. Для BlueStacks
  — обход конфликта версий ADB (выделенный HD-Adb + reconnect).
...
| `emu/` | менеджеры эмуляторов (MEmu / LDPlayer), ADB, ввод, скриншоты |
...
Каждый бот — отдельный worker-процесс ([`worker_run.py`](worker_run.py), запуск через
`run_from_source.py --worker`), чтобы инстансы не делили глобальное состояние.
